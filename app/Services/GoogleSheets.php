...
class GoogleSheets
{
    ...
    {
        $this->spreadSheetId = config('google_sheets.google_sheet_id');
    ...
    public function readGoogleSheet($sheet)
    {
        $dimensions = $this->getDimensions($this->spreadSheetId, $sheet);
        $range = $sheet.'!A1:' . $dimensions['colCount'];
    ...
    public function saveDataToSheet(array $data, $sheet)
    {
        $dimensions = $this->getDimensions($this->spreadSheetId, $sheet);
        $newID = [count($this->readGoogleSheet($sheet))];
        $finaldata = array_merge($newID, $data);
    ...
    private function getDimensions($spreadSheetId, $sheet)
    {
